...
// } else {console.log("You're incorrect.")};
console.log(`\nCandidate Name: ${candidateName}`);
for (let i = 0; i < questions.length; i ++) {
console.log(`${i + 1}) ${questions[i]}\nYour Answer: ${candidateAnswers[i]}\nCorrect Answer: ${correctAnswers[i]}\n`);
}
//need to check correct Answers with candidate Answer need to run a loop for it 
  let grade;
  let numOfCorrectAns = 0;
  for (i = 0; i < questions.length; i++) {
    if (String(candidateAnswers[1].toLowerCase()) === correctAnswers[1].toLowerCase()); {
      numOfCorrectAns = i + 1;
